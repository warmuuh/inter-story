...
var ZVM = (function () {
    ...
    */
    var Class = (function () {
        ...
        Class.subClass = function (prop) {
            var _super = this.prototype, proto, name, Klass;
    ...
    */
    ...
    if (DEBUG) {
        // Debug flags
        var debugflags = {}, get_debug_flags = function (data) {
            data = data.split(',');
            var i = 0;
            while (i < data.length) {
                debugflags[data[i++]] = 1;
    ...
    */
    ...
    var native_bytearrays = 0, ByteArray = native_bytearrays ?
    ...
    */
    ...
    Brancher = Opcode.subClass({
        ...
        post: function () {
            ...
            brancher = this.operands.pop(), offset = brancher[1];
    ...
    */
    ...
    function idiom_if_block(context, pc) {
        var i = 0, subcontext, sublen, brancher, lastop, secondlastop;
    ...
    */
    ...
    var Quetzal = IFF.subClass({
        ...
        write: function () {
            ...
            var pc = this.pc, ifhd = this.release.concat(this.serial, this.checksum, (pc >> 16) & 0xFF, (pc >> 8) & 0xFF, pc & 0xFF);
            // Add the chunks
            this.chunks = [
                { type: 'IFhd', data: ifhd },
    ...
    */
    var ZVMUI = Class.subClass({
        ...
        format: function () {
            var props = {}, temp, classes = [], fg = this.fg, bg = this.bg;
            if (this.bold) {
        ...
        process_colours: function () {
            ...
            fg_css = this.e.env.fgcolour, bg_css = this.e.env.bgcolour, 
            // Convert to true colour for storing in the header
            fg_true = fg_css ? convert_RGB(fg_css) : 0xFFFF, bg_true = bg_css ? convert_RGB(bg_css) : 0xFFFF, 
            // Search the list of standard colours
            fg = colours.indexOf(fg_true), bg = colours.indexOf(bg_true);
    ...
    */
    ...
    Indirect = Storer.subClass({
        ...
        /* sound_data */ 1014: Opcode.subClass({ brancher: 1 }),
    ...
    */
        ...
        */
        ...
        call: function (addr, storer, next, args) {
            ...
            var i, locals_count, old_locals_count = this.l.length, 
        ...
        copy_table: function (first, second, size) {
            size = U2S(size);
            var memory = this.m, i = 0, allowcorrupt = size < 0;
            size = Math.abs(size);
            // Simple case, zeroes
            if (second === 0) {
                while (i < size) {
        ...
        find_prop: function (object, property, prev) {
            var memory = this.m, this_property_byte, this_property, last_property = 0, 
        ...
        output_stream: function (stream, addr) {
            ...
            if (stream === -3) {
                var data = this.streams[2].shift(), text = this.text_to_zscii(data[1]);
        ...
        restart: function () {
            // Set up the memory
            var memory = ByteArray(this.data), version = memory.getUint8(0x00), addr_multipler = version === 5 ? 4 : 8, property_defaults = memory.getUint16(0x0A), extension = memory.getUint16(0x36);
        ...
        restore: function (data) {
            ...
            var quetzal = new Quetzal(data), qmem = quetzal.memory, qstacks = quetzal.stacks, pc = quetzal.pc, flags2 = this.m.getUint8(0x11), temp, i = 0, j = 0, call_stack = [], newlocals = [], newstack;
            // Memory chunk
            ...
            while (i < qstacks.length) {
                call_stack.unshift([
                    qstacks[i++] << 16 | qstacks[i++] << 8 | qstacks[i++],
        ...
        ret: function (result) {
            var call_stack = this.call_stack.shift(), storer = call_stack[1];
        ...
        save: function (pc, storer) {
            ...
            var memory = this.m, stack = this.s, locals = this.l, quetzal = new Quetzal(), compressed_mem = [], i, j, abyte, zeroes = 0, call_stack = this.call_stack.reverse(), frame, stack_len, stacks = [0, 0, 0, 0, 0, 0]; // Dummy call frame
            // IFhd chunk
            ...
            for (i = 0; i < call_stack.length; i++) {
                ...
                stacks.push(frame[0] >> 16, frame[0] >> 8 & 0xFF, frame[0] & 0xFF, // pc
        ...
        variable: function (variable, value) {
            var havevalue = value !== undefined, offset;
            if (variable === 0) {
                if (havevalue) {
                    this.s.push(value);
                }
    ...
    */
        ...
        */
        // These functions will be added to the object literal in api.js
        init_text: function () {
            var self = this, memory = this.m, alphabet_addr = memory.getUint16(0x34), unicode_addr = this.extension_table(3), unicode_len = unicode_addr && memory.getUint8(unicode_addr++);
            // Generate alphabets
        ...
        decode: function (addr, length) {
            var memory = this.m, start_addr = addr, temp, buffer = [], i = 0, zchar, alphabet = 0, result = [], resulttexts = [], usesabbr, tenbit, unicodecount = 0;
        ...
        encode: function (zscii) {
            var alphabets = this.alphabets, zchars = [], i = 0, achar, temp, result = [];
            // Encode the Z-chars
        ...
        parse_dict: function (addr) {
            var memory = this.m, addr_start = addr, dict = {}, entry_len, endaddr, 
        ...
        tokenise: function (text, buffer, dictionary, flag) {
            ...
            var memory = this.m, i = 2, textend = i + memory.getUint8(text + 1), letter, separators = dictionary.separators, word = [], words = [], wordstart = i, max_words, wordcount = 0;
        ...
        keyinput: function (data) {
            var charCode = data.charCode, keyCode = data.keyCode, 
    ...
    */
        ...
        */
        ...
        disassemble: function () {
            ...
            memory = this.m, temp, code, opcode_class, operands_type, // The types of the operands, or -1 for var instructions
            operands, 
        ...
        inputEvent: function (data) {
            var memory = this.m, code = data.code, response;
        ...
        // Run
        run: function () {
            var now = new Date(), pc, result, count = 0;
